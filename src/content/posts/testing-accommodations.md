...
### Case 2: ADHD on a Math Test

In the context of the same mathematics examination, ADHD presents a penumbral case. The exam-affecting impairments of ADHD are typically attributed to attention and working memory, but it is fundamentally unclear whether an "attention module" can be cleanly separated from the quantitative reasoning the test purports to measure. Furthermore, working memory probably does fall within the bounds of quantitative reasoning. In practice, it is difficult to know how much of a test-taker's impairment stems from a deficit in working memory versus a deficit in attention, or indeed, how separable those two constructs actually are.

The specific accommodation usually under consideration here is extra time. Problematically, we do not observe that extra time helps ADHD sufferers more than it helps neurotypicals. Even if extra time successfully assists an impaired, measurement-irrelevant module, time is a blunt instrument—it might simultaneously provide a broad boost to the very measurement-relevant modules the test is designed to assess. Because we cannot confidently isolate the accommodated module from the tested aptitude, the measurement prudence standard doesn't seem to give clear guidance about how to properly administer a math test to someone with ADHD.
